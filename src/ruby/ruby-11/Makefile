...
post-install:
	...
	cc -dynamiclib \
		-install_name /usr/lib/libruby.$(VERSION).dylib \
		-compatibility_version $(VERSION) -current_version $(VERSION) \
		-all_load -o $(DSTROOT)/usr/lib/libruby.$(VERSION).dylib \
		$(DSTROOT)/usr/lib/ruby/$(VERSION)/$(SYSSTRING)/libruby.a
	ln -s libruby.$(VERSION).dylib $(DSTROOT)/usr/lib/libruby.dylib
	...
	strip -x $(DSTROOT)/usr/lib/libruby.$(VERSION).dylib

   ...
   && (INTVAL (operands[3]) & CALL_LONG) == 0"
{
  ...
  /* APPLE LOCAL begin direct calls for objc_msgSend etc.  */
  if (DEFAULT_ABI == ABI_DARWIN)
    {
      ...
      if (note)
	{
	  sprintf (buf, "bla 0x%08x", INTVAL (XEXP (note, 0)));
	  return buf;
	}
    }
  /* APPLE LOCAL end direct calls for objc_msgSend etc.  */
  ...
  if (DEFAULT_ABI == ABI_DARWIN)
      ...
      if (note)
	{
	  if (which_alternative != 0)
	    abort ();
	  sprintf (buf, \"ba 0x%08x\", INTVAL (XEXP (note, 0)));
	  return buf;
	}
    }
  /* APPLE LOCAL end direct calls for objc_msgSend etc.  */

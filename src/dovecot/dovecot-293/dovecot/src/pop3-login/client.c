...
static void pop3_client_input(struct client *client)
{
	...
	while (!client->output->closed && !client->authenticating &&
		...
		/* APPLE */
		DTRACE_POP3_LOGIN_COMMAND_START(pop3_client, line, args);
		result = client_command_execute(pop3_client, line,
					   args != NULL ? args : "");
		DTRACE_POP3_LOGIN_COMMAND_FINISH(pop3_client, line, args,
						 result);
		if (result)

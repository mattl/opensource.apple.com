...
void fts_enqueue_update(struct mailbox *box)
{
	...
	str_append_c(path, '.');
	fts_escape(path, mailbox_get_vname(box));

	fd = open(str_c(path), O_WRONLY | O_CREAT | O_EXCL, 0600);
	if (fd < 0 && errno != EEXIST) {
		i_warning("fts: open(%s, O_CREAT) failed: %m", str_c(path));
		return;
	}
	close(fd);
}

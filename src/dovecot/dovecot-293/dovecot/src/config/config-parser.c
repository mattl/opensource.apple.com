			      ...
			      const char *key, const char *value)
{
	...
	case CONFIG_LINE_TYPE_KEYVARIABLE:
		...
		/* also set "foo_path = /path/to/file" for
		   "foo = </path/to/file" (needed for ssl_key) */
		if (type == CONFIG_LINE_TYPE_KEYFILE &&
		    strcmp(key, "ssl_key") == 0) {
			const char *path_key = t_strconcat(key, "_path", NULL);
			const char *path_line = t_strconcat(path_key, "=",
							    value, NULL);
			(void) config_apply_line(ctx, path_key, path_line,
						 NULL);
			/* ignore errors */
		}
#endif

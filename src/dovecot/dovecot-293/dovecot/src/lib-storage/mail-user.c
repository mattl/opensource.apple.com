...
static int mail_user_userdb_lookup_home(struct mail_user *user)
{
	...
	i_assert(!user->home_looked_up ||
		 !user->sieve_dir_looked_up);			/* APPLE */
	...
	if (ret > 0) {
		...
		for (fp = fields; fp != NULL && *fp != NULL; fp++) {
...
/* APPLE */
int mail_user_get_sieve_dir(struct mail_user *user, const char **sieve_dir_r)
{
	int ret;

	if (user->sieve_dir_looked_up) {
		*sieve_dir_r = user->_sieve_dir;

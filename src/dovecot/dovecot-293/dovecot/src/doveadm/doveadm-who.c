...
static void who_parse_line(const char *line, struct who_line *line_r)
{
	...
	const char *elapsed_str = args[3];			/* APPLE */
	const char *p, *ip_str;
	...
	line_r->elapsed = strtoul(elapsed_str, NULL, 10);	/* APPLE */
			       ...
			       const struct who_line *line)
{
	...
	/* APPLE */
	if (user->elapsed < line->elapsed)
		user->elapsed = line->elapsed;
...
#define ANVIL_CMD ANVIL_HANDSHAKE"CONNECT-DUMP\t-e\n"		/* APPLE */
...
static void who_print_user(struct who_context *ctx,		/* APPLE */
			   const struct who_user *user)
{
	const struct ip_addr *ip;
	const pid_t *pid;
	...
	if (ctx->dump_elapsed) {
		str_truncate(str, 0);
		str_printfa(str, "%lu", user->elapsed);
		doveadm_print(str_c(str));
	}
}
			   ...
			   const struct who_line *line)
{
	...
	for (i = 0; i < line->refcount; i++) T_BEGIN {
		...
		/* APPLE */
		if (ctx->dump_elapsed)
			doveadm_print(t_strdup_printf("%lu", line->elapsed));
	} T_END;
}
...
static void cmd_who(int argc, char *argv[])
{
	...
	if (!separate_connections) {
		...
		/* APPLE */
		if (ctx.dump_elapsed)
			doveadm_print_header_simple("elapsed");
...
struct doveadm_cmd doveadm_cmd_who = {
	cmd_who, "who",
				    /* APPLE */
	"[-a <anvil socket path>] [-1] [-e] [<user mask>] [<ip/bits>]"
};

                      ...
                      int *not_found_ptr)
{
  ...
  if ((minsym_sals.nelts > 0) && (sym_sals.nelts > 0))
    remove_duplicate_sals (&minsym_sals, sym_sals);
  ...
  for (i = 0; i < ret_sals.nelts; i++)
    {
      if (ret_sals.sals[i].symtab != NULL)
	{
	  char *filename = ret_sals.sals[i].symtab->filename;
	  if (copy != NULL)
	    {
	      canonical_name = xmalloc (strlen (filename) + strlen (copy) + 4);
	      sprintf (canonical_name, "%s:'%s'", filename, copy);
	    }
	  else
	    {
	      canonical_name = xmalloc (strlen (filename) + 30);
	      sprintf (canonical_name, "%s:%d", filename, 
		       ret_sals.sals[i].line);
	    }
